...
theorem makeInitialMatrix_basisPrefix (size : Nat) (scale : ZZ)
    (matrix : Generated.StrictRecombine.LLLMatrix)
    (hrun : Generated.StrictRecombine.makeInitialMatrix size scale =
      .ok matrix) :
    matrix.size = size ∧
      basisPrefixMatrix matrix size size =
        Matrix.diagonal (fun _ : Fin size => scale) := by
  have hprefix := makeInitialMatrix_prefix size scale matrix hrun
  refine ⟨hprefix.matrix_size, ?_⟩
  apply Matrix.ext
  intro row column
  simp only [basisPrefixMatrix, Matrix.diagonal_apply]
  have hentry := hprefix.entry row.val column.val row.isLt column.isLt
...
/-- Observable algebraic relation between the initial C++ lattice basis and
the synchronized `matrix`/`transform` pair maintained by LLL. -/
def LLLTransformRel (initial matrix transform :
...
theorem sizeReduceAt_preserves_mu_entry_of_source_lt
    (state output : Generated.StrictRecombine.LLLState)
    (source column : Nat)
    (hvalid : ConcreteLLLExecutionValid state)
    (hkMu : state.k < state.mu.size)
    (hsourceK : source < state.k)
    (hcolumn : column < state.mu[state.k]!.size)
    (hsourceColumn : source < column)
    (hrun : Generated.StrictRecombine.sizeReduceAt state source = .ok output) :
    ((output.mu[output.k]!)[column]!) =
      ((state.mu[state.k]!)[column]!) := by
  rcases sizeReduceAt_mu_eq state output source hvalid hsourceK hrun with
    ⟨coefficient, _hround, hmu⟩
  have hcontrol := sizeReduceAt_preserves_norms_k state output source hrun
  have hsourceMu : source < state.mu.size := lt_trans hsourceK hkMu
  have hsourceKRow : source < state.mu[state.k]!.size :=
    lt_trans hsourceColumn hcolumn
  have hlimitSource : source ≤ state.mu[source].size := by
    rw [hvalid.mu_rows_square source hsourceMu, ← hvalid.mu_size]
    exact Nat.le_of_lt hsourceMu
  have hentry := sizeReduceMuResult_target_get state.mu state.k source column
    coefficient hkMu hsourceMu
    (by simpa [getElem!_pos state.mu state.k hkMu] using hsourceKRow)
    hlimitSource
    (by simpa [getElem!_pos state.mu state.k hkMu] using hcolumn)
...
theorem basisPrefixMatrix_set_subtractRowArray
    (matrix : Generated.StrictRecombine.LLLMatrix)
    (target source size : Nat) (coefficient : ZZ)
    (hsize : matrix.size = size) (htarget : target < size)
    (hsource : source < size) :
    basisPrefixMatrix
        (matrix.set target
          (subtractRowArray matrix[target] matrix[source] coefficient size))
        size size =
      Matrix.transvection (⟨target, htarget⟩ : Fin size)
          (⟨source, hsource⟩ : Fin size) (-coefficient) *
        basisPrefixMatrix matrix size size := by
  funext row column
  by_cases hrow : row = (⟨target, htarget⟩ : Fin size)
  · subst row
    unfold basisPrefixMatrix
    rw [getElem!_pos _ target (by simpa [hsize])]
    simp only [Array.getElem_set, if_pos]
    rw [subtractRowArray_get _ _ _ _ column.val column.isLt]
    simp [Matrix.transvection, Matrix.add_mul]
    rw [getElem!_pos matrix target (by omega),
      getElem!_pos matrix source (by omega)]
    ring
  · unfold basisPrefixMatrix
    rw [getElem!_pos _ row.val (by simpa [hsize] using row.isLt)]
    have htargetRow : target ≠ row.val := by
      intro heq
      apply hrow
      exact Fin.ext heq.symm
    rw [Array.getElem_set_ne (by omega) (by omega) htargetRow]
    rw [Matrix.transvection, Matrix.add_mul, Matrix.one_mul]
    have hsingle := Matrix.single_mul_apply_of_ne (-coefficient)
      (⟨target, htarget⟩ : Fin size) (⟨source, hsource⟩ : Fin size)
      row column hrow (basisPrefixMatrix matrix size size)
    unfold basisPrefixMatrix at hsingle
    rw [Matrix.add_apply, hsingle, add_zero]
    rw [getElem!_pos matrix row.val (by omega)]

/-- One literal synchronized size-reduction preserves the algebraic meaning
of the C++ transform matrix. -/
theorem subtractMatrixRows_preserves_transform_rel
    (initial matrix transform outputMatrix outputTransform :
      Generated.StrictRecombine.LLLMatrix)
    (target source : Nat) (coefficient : ZZ)
  ...
  rw [hmatrixSize] at hmatrix htransform
  subst outputMatrix
  ...
  rw [basisPrefixMatrix_set_subtractRowArray transform target source initial.size
    coefficient htransformSize htarget hsource]
  rw [hrelation]
...
only update Gram--Schmidt metadata. -/
theorem sizeReduceAt_preserves_transform_rel
    (initial : Generated.StrictRecombine.LLLMatrix)
    (state output : Generated.StrictRecombine.LLLState) (source : Nat)
    (hrel : LLLTransformRel initial state.matrix state.transform)
    (hrun : Generated.StrictRecombine.sizeReduceAt state source = .ok output) :
    LLLTransformRel initial output.matrix output.transform := by
  unfold Generated.StrictRecombine.sizeReduceAt at hrun
  split at hrun
  next hk =>
    split at hrun
            ...
            | ok mu' =>
              ...
              next hk' =>
                ...
                next hsource' =>
                  have hout := Except.ok.inj hrun
                  subst output
                  exact subtractMatrixRows_preserves_transform_rel initial
                    state.matrix state.transform matrix' transform' state.k
                    source coefficient hrel hsubtract
                next hsource' => contradiction
              next hk' => contradiction
    next hsource => contradiction
  next hk => contradiction
...
theorem extraSizeReduceLoop_preserves_mu_entry_above
    ...
    (hvalid : ConcreteLLLExecutionValid state)
    (hkMu : state.k < state.mu.size)
    (hremaining : remaining ≤ column)
    (hcolumnK : column < state.k)
    (hrun : Generated.StrictRecombine.extraSizeReduceLoop remaining state =
      .ok output) :
    ((output.mu[output.k]!)[column]!) =
      ((state.mu[state.k]!)[column]!) := by
  ...
  | succ remaining ih =>
      ...
      | ok next =>
        simp only [hstep] at hrun
        have hsourceColumn : remaining < column := by omega
        have hsourceK : remaining < state.k := lt_trans hsourceColumn hcolumnK
        have hhead := sizeReduceAt_preserves_mu_entry_of_source_lt state next
          remaining column hvalid hkMu hsourceK
          (by
            simpa [getElem!_pos state.mu state.k hkMu,
              hvalid.mu_rows_square state.k hkMu, ← hvalid.mu_size] using
              (lt_trans hcolumnK hkMu))
          hsourceColumn hstep
        have hnextValid := sizeReduceAt_preserves_execution_valid state next
...
theorem lllStep_advanced_lovasz_at_previous
    (state output : Generated.StrictRecombine.LLLState)
    (hvalid : ConcreteLLLExecutionValid state)
    (hrun : Generated.StrictRecombine.lllStep state =
      .ok (.advanced output)) :
    ((3 : QQ) / 4 -
        ((output.mu[state.k]!)[state.k - 1]!) *
          ((output.mu[state.k]!)[state.k - 1]!)) *
        output.norms[state.k - 1]! ≤ output.norms[state.k]! := by
  rw [Generated.StrictRecombine.lllStep] at hrun
  by_cases hkPositive : 0 < state.k
  · rw [dif_pos hkPositive] at hrun
    by_cases hkMatrix : state.k < state.matrix.size
                  ...
                  | ok fullyReduced =>
                    ...
                    have hentry :=
                      extraSizeReduceLoop_preserves_mu_entry_above
                        ...
                        (reduced.k - 1) hreduceValid hkMu (by rfl)
                        (by omega) hextra
                    have hextraControl :=
                      extraSizeReduceLoop_preserves_norms_k
                        (reduced.k - 1) reduced fullyReduced hextra
                    have hentryState :
                        ((fullyReduced.mu[state.k]!)[state.k - 1]!) =
                          ((reduced.mu[state.k]!)[state.k - 1]!) := by
                      simpa only [hextraControl.2, hreduceControl.2] using hentry
                    have hout := Except.ok.inj hrun
                    injection hout with hstate
                    subst output
                    rw [hentryState, hextraControl.1]
                    rw [← hreduceControl.2]
...
sequentially. -/
theorem swapMatrixRows_pair_preserves_transform_rel
    (initial matrix transform outputMatrix outputTransform :
      Generated.StrictRecombine.LLLMatrix)
    (left right : Nat)
    (hrel : LLLTransformRel initial matrix transform)
    (hrunMatrix : Generated.StrictRecombine.swapMatrixRows matrix left right =
      .ok outputMatrix)
  ...
  have houtputTransformSize := swapMatrixRows_size transform outputTransform
    left right hrunTransform
  rcases swapMatrixRows_ok matrix outputMatrix left right hrunMatrix with
  ...
  let permutation := Equiv.swap
    ...
    (⟨right, hright⟩ : Fin initial.size)
  have hentry := congrArg
    (fun value => value (permutation row) column) hrelation
  unfold basisPrefixMatrix at hentry ⊢
  rw [swapMatrixRows_get_fin matrix outputMatrix left right initial.size
    hrunMatrix hleft hright (by omega) row]
  rw [Matrix.mul_apply]
  rw [swapMatrixRows_get_fin transform outputTransform left right initial.size
    hrunTransform hleft hright (by omega) row]
...
theorem lllStep_advanced_preserves_transform_rel
  ...
  · rw [dif_pos hkPositive] at hrun
    ...
    · rw [dif_pos hkMatrix] at hrun
      ...
      | ok reduced =>
        simp only [hreduce] at hrun
        have hreduced := sizeReduceAt_preserves_transform_rel initial state
          reduced (state.k - 1) hrel hreduce
        split at hrun
        next hkNorm =>
          split at hrun
          next hpredNorm =>
            ...
            next hkMu =>
              ...
              next hpredMu =>
                ...
                next hlovasz =>
                  ...
                  | ok fullyReduced =>
                    simp only [hextra] at hrun
                    have hfully := extraSizeReduceLoop_preserves_transform_rel
                      initial (reduced.k - 1) reduced fullyReduced hreduced
                      hextra
                    have hout := Except.ok.inj hrun
                    injection hout with hstate
                    subst output
...
theorem lllStep_swapped_preserves_transform_rel
    (initial : Generated.StrictRecombine.LLLMatrix)
    (state output : Generated.StrictRecombine.LLLState)
    (hvalid : ConcreteLLLExecutionValid state)
    (hrel : LLLTransformRel initial state.matrix state.transform)
                        ...
                        next hkSwapped =>
                          ...
                          next hpredSwapped =>
                            ...
                            let newNorm := reduced.norms[reduced.k] +
                              muOld * muOld * reduced.norms[reduced.k - 1]
                            let muNew :=
                              muOld * reduced.norms[reduced.k - 1] / newNorm
                            let correctedMu := swappedMu.set reduced.k
                              (swappedMu[reduced.k].set (reduced.k - 1) muNew)
                            cases hupdate :
                                Generated.StrictRecombine.updateMuAfterSwapLoop
                            ...
                            | ok finalMu =>
                              ...
                              injection hout with hstate
                              subst output
                              exact swapMatrixRows_pair_preserves_transform_rel
                                initial reduced.matrix reduced.transform matrix'
                                transform' reduced.k (reduced.k - 1) hreduced
                                hswapMatrix hswapTransform
                          next hpredSwapped => contradiction
                        next hkSwapped => contradiction
              next hpredMu => contradiction
            next hkMu => contradiction
...
theorem lllStep_preserves_transform_rel
    (initial : Generated.StrictRecombine.LLLMatrix)
    (state : Generated.StrictRecombine.LLLState)
    (branch : Generated.StrictRecombine.LLLStepResult)
    (hvalid : ConcreteLLLExecutionValid state)
    (hrel : LLLTransformRel initial state.matrix state.transform)
    (hrun : Generated.StrictRecombine.lllStep state = .ok branch) :
    LLLTransformRel initial branch.state.matrix branch.state.transform := by
  cases branch with
  | advanced output =>
      exact lllStep_advanced_preserves_transform_rel initial state output hrel hrun
  | swapped output =>
...
/-- The transform returned by the literal LLL initializer is the generated
integer identity matrix, hence already maps the input basis to itself. -/
theorem initializeLLL_transform_rel
    (matrix : Generated.StrictRecombine.LLLMatrix)
    (mu : Generated.StrictRecombine.QQMatrix) (norms : Array QQ)
    (transform : Generated.StrictRecombine.LLLMatrix)
    (hrun : Generated.StrictRecombine.initializeLLL matrix =
      .ok (mu, norms, transform)) :
    LLLTransformRel matrix matrix transform := by
  rw [Generated.StrictRecombine.initializeLLL] at hrun
  split at hrun
  next hsize =>
    cases htransform : Generated.StrictRecombine.makeInitialMatrix
        matrix.size 1 with
    | error fault => simp [htransform] at hrun
    | ok initialTransform =>
      simp only [htransform] at hrun
      cases hdot : Generated.StrictRecombine.dotRows matrix[0] matrix[0] with
      | error fault => simp [hdot] at hrun
      | ok dot =>
        simp only [hdot] at hrun
        let initialMu := Generated.StrictRecombine.zeroQQMatrix
          matrix.size matrix.size
        let initialNorms := Array.replicate matrix.size (0 : QQ)
        cases hgram : Generated.StrictRecombine.initializeGramSchmidtLoop
            matrix 1 initialMu
              (initialNorms.set 0 (dot : QQ) (by
                simp [initialNorms, hsize])) with
        | error fault =>
          rw [hgram] at hrun
          contradiction
        | ok gramOutput =>
          rcases gramOutput with ⟨finalMu, finalNorms⟩
...
/-- End-to-end transform equation for the literal generated `__lll_reduce`:
the returned integer transform maps the input lattice basis to the returned
reduced basis.  Short-row collection is executed afterward and cannot alter
either matrix. -/
theorem concreteLLLReduce_transform_rel
    (matrix : Generated.StrictRecombine.LLLMatrix)
    ...
    LLLTransformRel matrix output.val.1 output.val.2.1 := by
...
theorem assignCandidateClass_preserves_some
    (transform : Generated.StrictRecombine.LLLMatrix)
    (shortRows : Array Nat) (representative factorCount classId right : Nat)
    (classes output : Array (Option Nat))
    (hrun : Generated.StrictRecombine.assignCandidateClass transform shortRows
      representative factorCount classId right classes = .ok output)
    (column existingClass : Nat) (hcolumn : column < classes.size)
    (hmember : classes[column]! = some existingClass) :
    output[column]! = some existingClass := by
  induction hmeasure : factorCount - right using Nat.strong_induction_on
      generalizing right classes output with
  | h measure ih =>
      rw [Generated.StrictRecombine.assignCandidateClass] at hrun
        ...
        next hclass =>
          ...
          next hnone =>
            ...
            | ok equal =>
                ...
                by_cases hequalTrue : equal = true
                · subst equal
                  have hcolumnNe : column ≠ right := by
                    intro hcolumnEq
                    subst column
                    rw [getElem!_pos classes right hclass] at hmember
                    simp [hnone] at hmember
                  have hmemberSet :
                      (classes.set right (some classId))[column]! =
                        some existingClass := by
                    rw [getElem!_pos _ column (by simpa using hcolumn),
                      Array.getElem_set_ne hclass hcolumn (Ne.symm hcolumnNe),
                      ← getElem!_pos classes column hcolumn]
                    exact hmember
                  exact ih (factorCount - (right + 1)) (by omega) (right + 1)
                    (classes.set right (some classId)) output hrun
                    (by simpa using hcolumn) hmemberSet rfl
                · have hequalFalse := Bool.eq_false_of_not_eq_true hequalTrue
                  rw [hequalFalse] at hrun
                  exact ih (factorCount - (right + 1)) (by omega) (right + 1)
                    classes output hrun hcolumn hmember rfl
...
theorem partitionCandidateColumns_all_assigned
    (transform : Generated.StrictRecombine.LLLMatrix)
    (shortRows : Array Nat) (factorCount column classCount : Nat)
    (classes outputClasses : Array (Option Nat)) (outputCount : Nat)
    (hcolumn : column ≤ factorCount)
    (hprefix : ∀ position, position < column → position < classes.size →
      classes[position]! ≠ none)
    ...
    ∀ position, position < factorCount → position < outputClasses.size →
      outputClasses[position]! ≠ none := by
  ...
  | h measure ih =>
      ...
      next hnext =>
        ...
        next hclass =>
          split at hrun
          next existing =>
            apply ih (factorCount - (column + 1)) (by omega) (column + 1)
              classCount classes outputClasses outputCount (by omega)
            · intro position hposition hpositionSize
              by_cases hpositionEq : position = column
              · subst position
                rw [getElem!_pos classes column hclass]
                simp [existing]
          ...
          next hnone =>
            ...
            | ok assigned =>
                ...
                · intro position hposition hpositionSize
                  have hassignedSize := assignCandidateClass_size transform
                    shortRows column factorCount classCount (column + 1)
                    seeded assigned hassign
                  have hpositionSeeded : position < seeded.size := by omega
                  by_cases hpositionEq : position = column
                  · subst position
                    have hseededMember : seeded[column]! = some classCount := by
                      unfold seeded
                      rw [getElem!_pos _ column (by simpa),
                        Array.getElem_set_self]
                    have hpersist := assignCandidateClass_preserves_some
                      transform shortRows column factorCount classCount
                      (column + 1) seeded assigned hassign column classCount
                      (by simpa [seeded] using hclass) hseededMember
                    rw [hpersist]
                    simp
                  · have hold : classes[position]! ≠ none :=
                      hprefix position (by omega) (by simpa [seeded] using
                        hpositionSeeded)
                    cases hvalue : classes[position]! with
                    | none => exact (hold hvalue).elim
                    | some existingClass =>
                        have hseededMember : seeded[position]! =
                            some existingClass := by
                          rw [getElem!_pos _ position hpositionSeeded,
                            Array.getElem_set_ne hclass
                              (by simpa [seeded] using hpositionSeeded)
                              (Ne.symm hpositionEq),
                            ← getElem!_pos classes position
                              (by simpa [seeded] using hpositionSeeded)]
                          exact hvalue
                        have hpersist := assignCandidateClass_preserves_some
                          transform shortRows column factorCount classCount
                          (column + 1) seeded assigned hassign position
                          existingClass hpositionSeeded hseededMember
                        rw [hpersist]
                        simp
                · exact hrun
                · rfl
...
theorem partitionCandidateColumns_size
    ...
    (hrun : Generated.StrictRecombine.partitionCandidateColumns transform
      shortRows factorCount column classCount classes =
        .ok (outputClasses, outputCount)) :
    outputClasses.size = classes.size := by
  induction hmeasure : factorCount - column using Nat.strong_induction_on
      generalizing column classCount classes outputClasses outputCount with
  | h measure ih =>
      rw [Generated.StrictRecombine.partitionCandidateColumns] at hrun
      ...
      next hcolumn =>
        ...
        next hclass =>
          ...
          next existing =>
            exact ih (factorCount - (column + 1)) (by omega) (column + 1)
              classCount classes outputClasses outputCount hrun rfl
          next hnone =>
            let seeded := classes.set column (some classCount)
            cases hassign : Generated.StrictRecombine.assignCandidateClass
                transform shortRows column factorCount classCount (column + 1)
            ...
            | ok assigned =>
                ...
                have htail := ih (factorCount - (column + 1)) (by omega)
                  (column + 1) (classCount + 1) assigned outputClasses
                  outputCount hrun rfl
                have hassignSize := assignCandidateClass_size transform
                  shortRows column factorCount classCount (column + 1)
                  seeded assigned hassign
                simpa [seeded] using htail.trans hassignSize
        next hclass => contradiction
      next hcolumn =>
        have hout := Except.ok.inj hrun
        injection hout with hclasses hcount
...
theorem partitionCandidateColumns_from_empty_all_assigned
    (transform : Generated.StrictRecombine.LLLMatrix)
    (shortRows : Array Nat) (factorCount : Nat)
    (outputClasses : Array (Option Nat)) (outputCount : Nat)
    (hrun : Generated.StrictRecombine.partitionCandidateColumns transform
      shortRows factorCount 0 0 (Array.replicate factorCount none) =
        .ok (outputClasses, outputCount)) :
    outputClasses.size = factorCount ∧
      ∀ position, position < factorCount → outputClasses[position]! ≠ none := by
  have hsize := partitionCandidateColumns_size transform shortRows factorCount
    0 0 (Array.replicate factorCount none) outputClasses outputCount hrun
  simp only [Array.size_replicate] at hsize
  refine ⟨hsize, ?_⟩
  intro position hposition
  exact partitionCandidateColumns_all_assigned transform shortRows factorCount
    0 0 (Array.replicate factorCount none) outputClasses outputCount (by omega)
    (by intro position hposition; omega) hrun position hposition (by omega)
...
/-- Exact physical size accounting for the reverse erase loop: one active
entry is removed for every true consumed bit in the processed prefix. -/
theorem removeConsumedLoop_size_add_count
    (consumed : Array Bool) (remaining : Nat)
    (active output : Array Int32)
    (hremaining : remaining ≤ consumed.size)
    (hactiveRemaining : remaining ≤ active.size)
    (hrun : Generated.StrictRecombine.removeConsumedLoop consumed remaining
      active = .ok output) :
    output.size + (consumed.toList.take remaining).count true = active.size := by
  induction remaining generalizing active output with
  | zero =>
      rw [Generated.StrictRecombine.removeConsumedLoop] at hrun
      have hout := Except.ok.inj hrun
  ...
  | succ remaining ih =>
      ...
      have hactive : remaining < active.size := by omega
      have htake :
          (consumed.toList.take (remaining + 1)).count true =
            (consumed.toList.take remaining).count true +
              (if consumed[remaining] = true then 1 else 0) := by
        rw [List.take_succ]
        simp [List.getElem?_eq_getElem, hindex, Array.getElem_toList]
        cases hvalue : consumed[remaining] <;> simp [hvalue]
      by_cases hmarked : consumed[remaining] = true
      · rw [if_pos hmarked, dif_pos hactive] at hrun
        have htail := ih (active.eraseIdxIfInBounds remaining) output
          (by omega) (by simp [hactive]; omega) hrun
        rw [htake, if_pos hmarked]
        simp [hactive] at htail ⊢
        omega
      · rw [if_neg hmarked] at hrun
        have htail := ih active output (by omega) (by omega) hrun
        rw [htake, if_neg hmarked]
        simp only [add_zero]
        exact htail

theorem removeConsumed_size_add_count
    (active : Array Int32) (consumed : Array Bool) (output : Array Int32)
    (hsizes : consumed.size = active.size)
    (hrun : Generated.StrictRecombine.removeConsumed active consumed =
      .ok output) :
    output.size + consumed.count true = active.size := by
  unfold Generated.StrictRecombine.removeConsumed at hrun
  rw [dif_pos hsizes] at hrun
  have hsize := removeConsumedLoop_size_add_count consumed active.size active
    output (by omega) (Nat.le_refl _) hrun
  rw [← hsizes] at hsize
  have htakeFull : consumed.toList.take consumed.size = consumed.toList := by
    simpa using List.take_length consumed.toList
  rw [htakeFull, Array.count_toList] at hsize
  exact hsize.trans hsizes
...
/-- Marking candidate indices never clears an already-set consumed bit. -/
theorem markConsumedLoop_count_mono (candidate : Array Int32) (index : Nat)
    (consumed output : Array Bool)
    (hrun : Generated.StrictRecombine.markConsumedLoop candidate index consumed =
      .ok output) :
    consumed.count true ≤ output.count true := by
  induction hmeasure : candidate.size - index using Nat.strong_induction_on
      generalizing index consumed output with
  | h measure ih =>
      rw [Generated.StrictRecombine.markConsumedLoop] at hrun
...
theorem markConsumedLoop_count_lt_of_available
    (candidate : Array Int32) (consumed output : Array Bool)
    (hnonempty : 0 < candidate.size)
    (havailable : Generated.StrictRecombine.candidateAvailable candidate consumed =
      .ok true)
    (hrun : Generated.StrictRecombine.markConsumedLoop candidate 0 consumed =
      .ok output) :
    consumed.count true < output.count true := by
  have hvalid := candidateAvailable_true_valid candidate consumed havailable
  have hentry := hvalid 0 hnonempty
  have hfree : consumed[candidate[0].toInt64.toNat] = false := by
    unfold Generated.StrictRecombine.candidateAvailable at havailable
    rw [Generated.StrictRecombine.candidateAvailableLoop,
      dif_pos hnonempty, dif_pos hentry.1, dif_pos hentry.2] at havailable
    by_cases hused : consumed[candidate[0].toInt64.toNat] = true
    · simp [hused] at havailable
    · exact Bool.eq_false_of_not_eq_true hused
  rw [Generated.StrictRecombine.markConsumedLoop, dif_pos hnonempty,
    dif_pos hentry.1, dif_pos hentry.2] at hrun
  have htail := markConsumedLoop_count_mono candidate 1
    (consumed.set candidate[0].toInt64.toNat true) output hrun
  rw [Array.count_set hentry.2, hfree] at htail
  simp at htail
  omega
...
/-- Every physical validation push is paid for by at least one newly marked
consumed bit.  This is stronger than the candidate-count bound and composes
across repeated van-Hoeij rounds. -/
theorem validateCandidatesLoop_result_count_le_consumed
    (ops : Generated.StrictRecombine.CandidateValidationRawOps)
    (candidates : Array (Array Int32)) (candidateIndex : Nat)
    (activeLifted : Array SparsePolyZZ) (modulus : ZZ)
    (fStar fStar' : SparsePolyZZ) (result result' : Array SparsePolyZZ)
    ...
    (hrun : Generated.StrictRecombine.validateCandidatesLoop ops candidates
      candidateIndex activeLifted modulus fStar result consumed remaining =
        .ok (fStar', result', consumed')) :
    result'.size + consumed.count true ≤
      result.size + consumed'.count true := by
  induction hmeasure : candidates.size - candidateIndex using Nat.strong_induction_on
      generalizing candidateIndex fStar result consumed remaining fStar' result' consumed' with
  | h measure ih =>
      rw [Generated.StrictRecombine.validateCandidatesLoop] at hrun
                            ...
                            | ok quotientResult =>
                              ...
                              | ok consumedNext =>
                                ...
                                have hcandidateNonempty :
                                  ...
                                  simp [Array.isEmpty, hzero]
                                have hcountStrict :=
                                  markConsumedLoop_count_lt_of_available
                                    candidates[candidateIndex] consumed consumedNext
                                    hcandidateNonempty havailable hmark
                                have htail := ih
                                  (candidates.size - (candidateIndex + 1))
                                  (by omega) (candidateIndex := candidateIndex + 1)
                                  (fStar := quotientPrimitive)
...
theorem validateCandidates_result_count_le_consumed
    (ops : Generated.StrictRecombine.CandidateValidationRawOps)
    (fStar : SparsePolyZZ) (activeLifted : Array SparsePolyZZ) (modulus : ZZ)
    (candidates : Array (Array Int32)) (result : Array SparsePolyZZ)
    (fStar' : SparsePolyZZ) (result' : Array SparsePolyZZ)
    ...
    (hrun : Generated.StrictRecombine.validateCandidates ops fStar activeLifted
      modulus candidates result = .ok (fStar', result', consumed)) :
    result'.size ≤ result.size + consumed.count true := by
  unfold Generated.StrictRecombine.validateCandidates at hrun
  have hbound := validateCandidatesLoop_result_count_le_consumed ops candidates 0
    activeLifted modulus fStar fStar' result result'
    (Array.replicate activeLifted.size false) consumed activeLifted.size hrun
...
/-- One complete successful extraction round cannot increase the sum of
accumulated physical results and remaining active indices. -/
theorem validateRemove_result_active_size_le
    (ops : Generated.StrictRecombine.CandidateValidationRawOps)
...
/-- Candidate validation simultaneously preserves the live canonical
remainder and L2 nonzeroness of every physical result factor. -/
theorem validateCandidatesLoop_result_ne_zero
    (ops : Generated.StrictRecombine.CandidateValidationRawOps)
    (candidates : Array (Array Int32)) (candidateIndex : Nat)
    (activeLifted : Array SparsePolyZZ) (modulus : ZZ)
    (fStar fStar' : SparsePolyZZ) (result result' : Array SparsePolyZZ)
    (consumed consumed' : Array Bool) (remaining : Nat)
    (hcanonical : StrictPolynomialMod.SparsePolyZZCanonical fStar)
    (hresult : ∀ factor ∈ result.toList, SparsePolyZZ.toPoly factor ≠ 0)
    (hrun : Generated.StrictRecombine.validateCandidatesLoop ops candidates
      candidateIndex activeLifted modulus fStar result consumed remaining =
        .ok (fStar', result', consumed')) :
    StrictPolynomialMod.SparsePolyZZCanonical fStar' ∧
      ∀ factor ∈ result'.toList, SparsePolyZZ.toPoly factor ≠ 0 := by
  induction hmeasure : candidates.size - candidateIndex using Nat.strong_induction_on
      generalizing candidateIndex fStar result consumed remaining fStar' result' consumed' with
  | h measure ih =>
      rw [Generated.StrictRecombine.validateCandidatesLoop] at hrun
                        ...
                        | ok divResult =>
                          ...
                          · simp only [hremainder, if_true] at hrun
                            ...
                            | ok quotientResult =>
                              ...
                              | ok consumedNext =>
                                simp only [hmark] at hrun
                                have hfactorNe := validationRecoveredFactor_ne_zero
                                  ops candidates[candidateIndex] activeLifted modulus
                                  fStar product symmetric factor quotient #[] content
                                  hfstar hcanonical hproduct hsymmetric hprimitive
                                  hdivmod
                                have hresultPush : ∀ candidateFactor ∈
                                    (result.push factor).toList,
                                    SparsePolyZZ.toPoly candidateFactor ≠ 0 := by
                                  intro candidateFactor hcandidateFactor
                                  rw [Array.toList_push] at hcandidateFactor
                                  rcases List.mem_append.mp hcandidateFactor with
                                    hprefix | hlast
                          ...
                          · simp only [hremainder, if_false] at hrun
                            exact ih
                              ...
                              (consumed := consumed) (remaining := remaining)
                              (fStar' := fStar') (result' := result')
                              (consumed' := consumed') hcanonical hresult hrun rfl
                next hfstar => contradiction
      next hcandidates =>
        have hout := Except.ok.inj hrun
        cases hout
...
theorem validateCandidates_result_ne_zero
    (ops : Generated.StrictRecombine.CandidateValidationRawOps)
    (fStar : SparsePolyZZ) (activeLifted : Array SparsePolyZZ) (modulus : ZZ)
    (candidates : Array (Array Int32)) (result : Array SparsePolyZZ)
    (fStar' : SparsePolyZZ) (result' : Array SparsePolyZZ)
    ...
    StrictPolynomialMod.SparsePolyZZCanonical fStar' ∧
      ∀ factor ∈ result'.toList, SparsePolyZZ.toPoly factor ≠ 0 := by
  unfold Generated.StrictRecombine.validateCandidates at hrun
  exact validateCandidatesLoop_result_ne_zero ops candidates 0 activeLifted
    modulus fStar fStar' result result'
    (Array.replicate activeLifted.size false) consumed activeLifted.size
