...
- 证明 UInt64 `(q+1)*d` 精确观察为数学估计倍数 `%B`，包括 `q+1=B`
  时先回绕为零的边界情况。
- 证明 UInt64 初始减法的完整分情况：估计倍数不高于输入时结果为
  `N-Q`；估计倍数高于输入时结果为 `B-(Q-N)`。两种情况都通过
  `Nat.ModEq` 加一个基数后做安全减法，再由严格窗口唯一化代表元。
- 证明完整机器 correction block 的 Nat 观察模型，包括 add-back 的 `%B`
  与最终无下溢条件减法。
- 组合全部局部引理，闭合单轮规范化预逆归约：在 `u1<pn`、规范化界及
  预逆乘法双边界下，真实 `preinvRoundIR` 精确返回
  `(u1*B+u0)%pn`。该结论保留生成代码中的整数 cast、所有 UInt64 回绕
  和两个 correction 分支。
- 闭合生成 `nmod_mul_ir`：证明输入左移无回绕、UInt128 乘积高 limb
  小于规范化模数、单轮归约适用，并用
