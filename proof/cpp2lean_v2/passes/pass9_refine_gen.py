"""Pass 9: emit the centralized, fully proved public refinement contracts.

The input MIR list is accepted for pipeline compatibility, but public output
is determined by ``REFINEMENT_MAP``.  This pass never emits theorem skeletons,
``sorry`` declarations, or semantic fallbacks.  The generated theorem names
retain the original C++ entry names, including their leading double
underscores, and wrap the checked strict L1-to-L2 proofs.
"""
...
theorem {theorem_name}
    ...
    ∃ factors,
      Generated.StrictSquarefreeZp.__squarefree_Zp_raw_ir
          (StrictSquarefreeEntry.strictSQFRawOps this hcfg physical)
...
def _emit_verified_contracts_file(infos: list[dict]) -> str:
    ...
    # Lead with the two user-facing end-to-end results.  Component contracts
    # follow in source-pipeline order, so opening Generated.lean presents the
    # conclusion before its implementation detail.
    contract_order = {
        "strict_factor_zp": 0,
        "strict_factor_zz": 1,
        "strict_select_prime": 2,
        "strict_squarefree_zp": 3,
        "strict_ddf_zp": 4,
        "strict_edf_zp": 5,
        "strict_hensel_eea": 6,
        "strict_hensel_step": 7,
        "strict_hensel_lift_recursive": 8,
        "strict_hensel_extract_factors": 9,
        "strict_hensel_tree_build": 10,
        "strict_hensel_lift_upoly": 11,
    }
    contracts.sort(key=lambda info: contract_order[
        info["verified_contract"]["kind"]])
...
def refine_gen_pass(_top_funcs: list[object] | None = None) -> None:
