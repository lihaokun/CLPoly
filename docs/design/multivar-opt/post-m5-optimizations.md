...
### 单变量因式分解大素数（__select_prime）

当前 `__select_prime` 使用 `boost::math::prime()` 表中的小素数（最大 ~10⁵），
逐个尝试找到使 f mod p 无平方且因子数最少的素数。

**问题**：小素数空间有限，对高次多项式（deg≥50）可能需要大量尝试。
`__select_prime` 是单变量因式分解的主要瓶颈（占 77-94% 时间）。

**优化方向**：使用大（如 63-bit 机器）素数。大素数下 DDF/EDF 更快（模运算避免
大数溢出），且 f mod p 更可能保持无平方（坏素数概率 ~ d/p → 大 p 时极低）。

**需要解决的问题**：
1. 大素数生成：自行实现 Miller-Rabin（参考 NTL `GenPrime`：候选生成 → 小素数试除 → Miller-Rabin），
   或直接使用 GMP 的 `mpz_nextprime`
2. DDF 中 `x^p mod f(x)` 对大 p 需要高效平方-乘法链（当前 `__upoly_powmod` 已支持）
3. EDF 中随机多项式 `(r^((p-1)/2) - 1) mod f`：p 大时指数也大，需高效 powmod
4. Hensel 提升精度 `p^a > 2B`：大 p 意味着更少的提升步数

**状态**：待调研。Zp 64-bit 扩展（M6a）已为此铺路。
