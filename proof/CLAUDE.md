...
1. **数学正确性**：每个数学声明（等式、不等式、整除性、互素性）是否正确？反证法的矛盾是否真实？归纳的 base/step 是否完整？
2. **无跳步**：每一步是否从已知条件或前序步骤直接推出？是否有"显然"、"同理"等未展开的声明？若有，展开验证其正确性。
3. **Lean 可形式化**：每一步是否有明确的 Lean tactic/lemma 路径？列出的 Mathlib API 是否存在且签名正确？`by_contra`/`push_neg` 的结构是否能在 Lean 中实现？
4. **工程问题已解决**：`termination_by` 的度量是否严格递减？`decreasing_by` 中的 bound 是否在 context 中可用？`set`-bound 变量是否影响 `rw`？
5. **边界情况**：f = 0, f = 1, 空列表, d = 0 等退化情况是否处理？前置条件是否排除所有不可证的边界？
