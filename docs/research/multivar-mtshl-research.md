...
MTSHL-d（CASC 2018）虽然解决了 MDP 问题，但在 r 个 Zp 因子对应 s>1 个真因子时仍需 Zassenhaus 重组。
实践中，随机选取的求值点使得 s=1 以高概率成立（单变量像因子数 = 真因子数），
因此 Zassenhaus 通常只需测试 s=1 即成功。

> **勘误**：此前文档声称"Maple 2019 通过双变量基底 Zp[x1,x2] 保证 s=1"，
> 经核实 Monagan & Tuncer 全部 5 篇论文（CASC 2016/2018, ICMS 2018, MC 2019, JSC 2020），
> 均使用**单变量基底** Zp[x1]，从未描述双变量基底算法。该声明为错误推断，已删除。
