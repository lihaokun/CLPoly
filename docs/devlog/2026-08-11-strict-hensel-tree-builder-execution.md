...
A canonical `HenselLiftTree` is now generated by the same well-founded
interval split and preorder append rule as the C++ builder.  It contains only
indices and branch shape.  The refinement proof computes the exact output
array size—not merely a lower bound—and proves it equals this topology's
`nodeCount`.  Thus the concrete execution neither skips a recursive allocation
