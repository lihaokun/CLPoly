...
### 3.2.1 Lean 形式化路径（避免 Step 5 的 UFD 展开）

**关键洞察**：用**反证法**可以完全避免 Step 5（逐因子导数分析）。直接证 `derivative(c_rem) ≠ 0 → ⊥`。

**反证法路径**：
```
假设 derivative(c_rem) ≠ 0。
(a) squarefree_div_gcd_derivative(c_rem):
    w' := normalize(c_rem /ₘ normalize(gcd(c_rem, c_rem'))) 是 Squarefree。
    derivative(c_rem) ≠ 0 → gcd(c_rem, c_rem') 是真因子 → deg(w') ≥ 1。
