...
- **Mignotte bound**：g | f → ‖g‖_∞ ≤ B_Mig(f)（1 sorry，作为引理声明）
- **Mignotte 精度条件 (M)**：m > 2ℓ · B_Mig(f)
...
```
设 F ∈ Z_m[x]（m = p^k，k ≥ 1），Ā, B̄ ∈ F_p[x] 首一，gcd(Ā, B̄) = 1。
若 (A₁, B₁) 和 (A₂, B₂) 都满足：
  Aᵢ · Bᵢ ≡ F (mod m)
...
**注**：关键条件是 **B 首一 in Z_m[x]**。仅 "B̄ 首一 in F_p[x]" 加 deg 保持不够（见 §2.2 证明中 c ≠ 0 的分析）。
在 lc-baking 设定中，Hᵢ (i ≥ 2) 首一 in Z_m[x]（(I3)），故唯一性条件满足。
...
| 组件 | sorry | 说明 |
|------|-------|------|
| `mignotte_bound` | **1** | ‖g‖_∞ ≤ B_Mig(f)（Landau-Mignotte 不等式） |

### 形式化顺序与行数

| Step | 内容 | 估计行数 |
|------|------|---------|
| 1 | `mignotte_bound` 接口 + sorry | ~10 |
