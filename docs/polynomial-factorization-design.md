...
```
// 将 factors mod (xₖ - αₖ)^j 提升到 mod (xₖ - αₖ)^(j+1)
...
for j = 1 to dₖ:
    ...
    e ← f* - g₁ · g₂ · ··· · gᵣ
    // 提取 (xₖ - αₖ)^j 的系数
    eⱼ ← coeff(e, (xₖ - αₖ)^j)    // 通过 Taylor 展开或多项式除法提取
...
- 步骤 1：改为使用 lex 首变量（匹配 `cont()` 实现），删除 "度数最高" 的选择策略
- 步骤 5：删除 "可能关于其他变量可分解" 的错误注释
- 步骤 6：新增失败重试机制
- 步骤 7：传入 `lc_assignments` 信息
- 步骤 8：新增去缩放（取本原部分），对**原始 f**（而非缩放后的）试除
...
以下函数是 M5 所需但在 §4 辅助函数层和现有代码库中尚不存在的：
...
#### 9.10.2 `__poly_coeff_l1_norm` — 多变量系数 L1 范数
...
当前 `factorize`（§8.5）在 `vars.size() > 1` 时抛异常。M5 完成后，应修改为：

```cpp
// §8.5 factorize (lex 特化) — 修订
template<class var_order>
factorization<polynomial_<ZZ, lex_<var_order>>>
factorize(const polynomial_<ZZ, lex_<var_order>>& F)
{
...
QQ[x₁,...,xₙ] 入口（§8.7）无需修改——其内部先转换为 ZZ 多项式再调用
`factorize(ZZ)`，转换逻辑对多变量同样有效。

### 9.12 与 `squarefreefactorize` 的兼容性

现有 `squarefreefactorize`（`polynomial_gcd.hh:101`）对 lex 首变量求导，
递归处理内容。对多变量多项式，它能正确检测**关于首变量**的重因子。

**已知限制：** 如果 f 关于非首变量有重因子（如 `f = g(x,y)² · h(x,y)` 且
g 不整除 ∂f/∂x），`squarefreefactorize` 可能将其视为无平方。
但这对 Wang 算法不构成问题——Wang 算法的前置条件只要求
**单变量像** `f(x₁, α...)` 无平方，这在 `__select_eval_point` 中已保证。
...
| **Phase 5** | M5: 多变量 Wang | `pp`, `__select_eval_point`, `__wang_leading_coeff` (含 `__wang_lc_result`), `__multivar_hensel_lift`, `__factor_multivar`, `factorize` 多变量 dispatch | M4 |
...
| Phase 5 | `pp(f)` | 验证 `cont(f) · pp(f) == f` |
| Phase 5 | `__select_eval_point` | 验证返回点满足三个条件 (无平方, lc 非零, 度数不降) |
| Phase 5 | `__wang_leading_coeff` | 构造已知分解的多项式，验证 lc 分配正确 |
| Phase 5 | `__multivar_hensel_lift` | 对二变量多项式，验证提升后试除正确 |
| Phase 5 | `__factor_multivar` | 与 Mathematica `Factor[f]` 对比 |
| Phase 5 | `factorize` (多变量) | 与 Mathematica `FactorList[f]` 对比 (二变量+三变量) |
...
| 多变量主变量 | lex 首变量 (非最高度数) | 匹配 `cont()` 实现，避免变量重排复杂度 |
| Wang lc 校正返回值 | `__wang_lc_result` 结构体 | 需返回成功标志 + 缩放后的 f + lc 分配列表 |
| 多变量 Hensel 提升 | 逐变量线性提升 | 标准 Wang 方式，每变量提升到 `deg(f,xₖ)` 阶 |
| Wang 失败处理 | 换求值点重试 (最多 10 次) | 简单可靠；远期可加 EEZ-Wang |
