#!/usr/bin/env python3
"""
survey_ast.py — 调研 67 个目标函数的 Clang AST，输出构造统计 + 生成 cache。

由 cpp2lean v1 迁移到 v2（2026-05-18），路径常量同步 PR #20 的目录调整。
用途：为 translator v2 的 Pass 1-8 smoke tests 提供 AST cache。

输出：
  docs/design/l1-translation-validation/survey/
    ├── ast-kinds.md       — 所有 AST node kind 的直方图
    ├── operators.md        — 运算符直方图
    ├── types.md            — 类型直方图
    └── summary.md          — 总览

缓存：
  proof/cpp2lean_v2/tests/ast_cache/<func_name>.json — 每函数的 Clang AST
                                                      （去除 base64 binary 字段）

用法：
  cd proof/cpp2lean_v2
...
# ----------------------------------------------------------------------
# 路径配置（cpp2lean_v2 layout）
# ----------------------------------------------------------------------
SCRIPTS_DIR = Path(__file__).resolve().parent       # cpp2lean_v2/scripts
V2_ROOT = SCRIPTS_DIR.parent                        # cpp2lean_v2
PROJECT_ROOT = V2_ROOT.parent.parent                # repo root

INSTANTIATE_CC = V2_ROOT / "tests" / "fixtures" / "instantiate.cc"
AST_CACHE_DIR = V2_ROOT / "tests" / "ast_cache"
SURVEY_DIR = PROJECT_ROOT / "docs" / "design" / "l1-translation-validation" / "survey"


# ----------------------------------------------------------------------
# 目标函数清单（从 cpp2lean_v2/class_map.py 复用）
# ----------------------------------------------------------------------
