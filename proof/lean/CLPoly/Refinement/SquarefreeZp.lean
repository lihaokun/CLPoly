...
/-- Totality of the generated equal-degree heap loop.  The proof constructs
each root consume and checked extract, then recurses on the exact smaller heap
size while transporting the complete concrete representation state. -/
theorem pairVecDivVHCConsumeEqualDegree_succeeds
    (this : DenseUPolyZp) (p degreeLimit dividendIndex : Nat)
    (dividend : SparsePolyZp) (sourceHeap : Array Nat)
    (sourceNodes : Array PairVecDivVHCNode)
    (frontier : PairVecDivVHCFrontier)
    (heap : Array Nat) (k : UInt64) (nodes : Array PairVecDivVHCNode)
    (lin : Array Nat) (resetH : Nat) (owners : Nat → Finset Nat)
    (quotient divisor : SparsePolyZp)
    (hsourceSelect : pairVecDivVHCSelectFrontier dividendIndex dividend
      sourceHeap sourceNodes = .ok frontier)
    (hownership : PairVecDivVHCHeapChainOwnership heap owners nodes)
    (hseparated : ∀ (slot head : Nat), heap[slot]? = some head →
      Disjoint lin.toList.toFinset (owners head) ∧
        head ∉ lin.toList.toFinset)
    ...
    (hquotient : SparsePolyZp.Canonical p quotient)
    (hdivisor : SparsePolyZp.Canonical p divisor) :
    ∃ result, pairVecDivVHCConsumeEqualDegree this frontier.degree heap k nodes
      lin resetH quotient divisor = .ok result := by
  induction hsize : heap.size using Nat.strong_induction_on generalizing heap k
      nodes lin resetH owners with
  | h size ih =>
      rw [pairVecDivVHCConsumeEqualDegree]
      by_cases hheap : 0 < heap.size
      · simp only [hheap, ↓reduceDIte]
        rcases hownership.heapPointersValid heap owners nodes 0 hheap with
          ⟨rootIndex, rootNode, rootMono, hrootIndex, hrootNode, hrootActive⟩
        have hrootEq : heap[0] = rootIndex := by
          rw [Array.getElem?_eq_getElem hheap] at hrootIndex
          exact Option.some.inj hrootIndex
        have hmono : pairVecDivVHCMono heap[0] nodes = .ok rootMono :=
          (pairVecDivVHCMono_eq_ok_iff heap[0] nodes rootMono).2 (by
            rw [hrootEq]
            exact ⟨rootNode, hrootNode, hrootActive⟩)
        simp only [hmono]
        by_cases hequal : rootMono.deg = frontier.degree
        · simp only [hequal, ↓reduceDIte]
          have hselect := pairVecDivVHCSelectFrontier_eq_of_root_degree_eq
            dividendIndex dividend sourceHeap heap sourceNodes nodes frontier
            rootMono hheap hmono hequal hsourceSelect
          have hrootOwns :=
            pairVecDivVHCHeapChainOwnership_root_owns heap owners nodes
              hownership hheap
          have hrootDegree := hhomogeneous 0 heap[0] rootMono
            (Array.getElem?_eq_getElem hheap) hmono
          have hchainDegree : PairVecDivVHCChainAtDegree (some heap[0])
              (Finset.range nodes.size) nodes frontier.degree := by
            rw [← hequal]
            exact pairVecDivVHCChainAtDegree_mono (some heap[0])
              (owners heap[0]) (Finset.range nodes.size) nodes rootMono.deg
              hrootDegree (pairVecDivVHCChainOwns_subset_range _ _ _ hrootOwns)
          rcases pairVecDivVHCConsumeRootBucket_succeeds this p dividendIndex k
              dividend heap nodes lin resetH owners quotient divisor frontier
              hheap ⟨owners, hownership, hcovered⟩ hownership hhomogeneous
              hordered hready hlinReady hlinBelow hfixed hdenotes hquotient
              hdivisor hselect hchainDegree with ⟨bucket, hconsume⟩
          simp only [dif_pos trivial, hconsume]
          rcases pairVecDivVHCConsumeRootBucket_extractChecked_succeeds this
              heap k nodes lin resetH quotient divisor bucket owners hheap
              hownership hconsume with ⟨extracted, hchecked⟩
          rw [hchecked]
          have hextract := pairVecDivVHCExtractChecked_raw heap bucket.nodes
            extracted hchecked
          have hrootValid := pairVecDivVHCChainOwns_valid (some heap[0])
            (owners heap[0]) nodes hrootOwns
          have hchainValid := pairVecDivVHCChainValid_mono (some heap[0])
            (owners heap[0]) (Finset.range nodes.size) nodes hrootValid
            (pairVecDivVHCChainOwns_subset_range _ _ _ hrootOwns)
          rcases pairVecDivVHCConsumeRootBucket_preserves_node_invariants this
              p degreeLimit heap k nodes lin resetH quotient divisor bucket
              hheap hquotient hactiveBelow hdenotes hfixed hready hchainValid
              hconsume with ⟨hactiveBelow', hdenotes', hfixed', hready'⟩
          have hlinBelow' :=
            pairVecDivVHCConsumeRootBucket_preserves_linBelow this p
              frontier.degree heap k nodes lin resetH quotient divisor bucket
              (owners heap[0]) hheap hrootOwns (by simpa [hequal] using
                hrootDegree) (hseparated 0 heap[0]
                  (Array.getElem?_eq_getElem hheap)).1 hlinBelow hdenotes
              hquotient hconsume
          have hownershipOld :=
            pairVecDivVHCConsumeRootExtract_preserves_heapChainOwnership this
              heap extracted.1 k nodes lin resetH quotient divisor bucket
              owners hheap hownership hconsume hextract
          have hcoveredOld :=
            pairVecDivVHCConsumeRootExtract_preserves_nodesCovered this heap
              extracted.1 k nodes lin resetH quotient divisor bucket owners
              hheap hownership hcovered hconsume hextract
          have hhomogeneousOld :=
            pairVecDivVHCConsumeRootExtract_preserves_heapChainsHomogeneous
              this heap extracted.1 k nodes lin resetH quotient divisor bucket
              owners hheap hownership hhomogeneous hconsume hextract
          have hordered' :=
            pairVecDivVHCConsumeRootExtract_preserves_heapOrdered this heap
              extracted.1 k nodes lin resetH quotient divisor bucket owners
              hheap hownership hordered hconsume hextract
          have haway' :=
            pairVecDivVHCConsumeRootExtract_preserves_away_linReady this heap
              extracted.1 k nodes lin resetH quotient divisor bucket owners
              hheap hownership hseparated hlinReady hconsume hextract
          rcases haway'.1 with ⟨nextOwners, hnextOwnership, hnextSeparated⟩
          have hnextCovered := hcoveredOld.congr_owners extracted.1 owners
            nextOwners bucket.lin bucket.resetH bucket.nodes hownershipOld
            hnextOwnership
          have hnextHomogeneous := hhomogeneousOld.congr_owners extracted.1
            owners nextOwners bucket.nodes hownershipOld hnextOwnership
          have hsmaller : extracted.1.size < size := by
            omega
          rcases ih extracted.1.size hsmaller extracted.1 bucket.coefficient
              bucket.nodes bucket.lin bucket.resetH nextOwners
              hnextOwnership hnextSeparated hnextCovered hnextHomogeneous
              hordered' hactiveBelow' hdenotes' hfixed' hready' haway'.2
              hlinBelow' rfl with ⟨result, hrun⟩
          exact ⟨result, hrun⟩
        · simp only [hequal, ↓reduceDIte]
          exact ⟨_, rfl⟩
      · simp only [hheap, ↓reduceDIte]
...
/-- A successful source heap insertion only rewrites one allocated VHC node;
it never changes the separately allocated node-block length. -/
theorem pairVecDivVHCInsert_nodes_size
    (newNode : Nat) (heap heap' : Array Nat)
    (nodes nodes' : Array PairVecDivVHCNode)
    (hrun : pairVecDivVHCInsert newNode heap nodes = .ok (heap', nodes')) :
    nodes'.size = nodes.size := by
  rcases pairVecDivVHCInsert_nodes_result newNode heap heap' nodes nodes' hrun
    with ⟨next, hset⟩
  exact pairVecDivVHCSetNext_nodes_size newNode next nodes nodes' hset

theorem pairVecDivVHCConsumeRootBucket_nodes_size
    (this : DenseUPolyZp) (heap : Array Nat) (k : UInt64)
    (nodes : Array PairVecDivVHCNode) (lin : Array Nat) (resetH : Nat)
    (quotient divisor : SparsePolyZp) (result : PairVecDivVHCBucketResult)
    (hrun : pairVecDivVHCConsumeRootBucket this heap k nodes lin resetH
  ...
  unfold pairVecDivVHCConsumeRootBucket at hrun
  split at hrun <;> try contradiction
  exact pairVecDivVHCConsumeChain_nodes_size this (some heap[0])
    (Finset.range nodes.size) k nodes lin resetH quotient divisor result hrun

theorem pairVecDivVHCConsumeEqualDegree_nodes_size
    (this : DenseUPolyZp) (degree : Nat) (heap : Array Nat) (k : UInt64)
    (nodes : Array PairVecDivVHCNode) (lin : Array Nat) (resetH : Nat)
    (quotient divisor : SparsePolyZp)
    (result : PairVecDivVHCEqualDegreeResult)
              ...
              | ok bucket =>
                  ...
                  | ok extracted =>
                      ...
                      have hsmaller : extracted.1.size < size := by
                        rw [← hsize]
                        omega
                      have htail := ih extracted.1.size hsmaller extracted.1
                        bucket.coefficient bucket.nodes bucket.lin bucket.resetH
                        result hrun rfl
                      exact htail.trans
                        (pairVecDivVHCConsumeRootBucket_nodes_size this heap k
                          nodes lin resetH quotient divisor bucket hconsume)
            · simp only [hequal, ↓reduceDIte, Except.ok.injEq] at hrun
              subst result
              rfl
      · simp only [hheap, ↓reduceDIte, Except.ok.injEq] at hrun
...
theorem pairVecDivVHCReinsertLin_nodes_size
    (heap : Array Nat) (nodes : Array PairVecDivVHCNode) (lin : Array Nat)
    (result : PairVecDivVHCHeapState)
    (hrun : pairVecDivVHCReinsertLin heap nodes lin = .ok result) :
    result.nodes.size = nodes.size := by
  induction hsize : lin.size using Nat.strong_induction_on generalizing heap
      nodes lin result with
  | h size ih =>
      rw [pairVecDivVHCReinsertLin] at hrun
      by_cases hlin : 0 < lin.size
      · simp only [hlin, ↓reduceDIte] at hrun
        cases hinsert : pairVecDivVHCInsert lin[lin.size - 1] heap nodes with
        | error fault => simp [hinsert] at hrun
        | ok inserted =>
            rcases inserted with ⟨heap', nodes'⟩
            rw [hinsert] at hrun
            have hsmaller : lin.pop.size < size := by
              simp only [Array.size_pop]
              rw [← hsize]
              omega
            have htail := ih lin.pop.size hsmaller heap' nodes' lin.pop result
              hrun rfl
            exact htail.trans (pairVecDivVHCInsert_nodes_size
              lin[lin.size - 1] heap heap' nodes nodes' hinsert)
      · simp only [hlin, ↓reduceDIte, Except.ok.injEq] at hrun
        subst result
        rfl

theorem pairVecDivVHCActivateReset_nodes_size
    (resetH : Nat) (heap : Array Nat) (nodes : Array PairVecDivVHCNode)
    (quotient divisor : SparsePolyZp) (result : PairVecDivVHCHeapState)
    (hrun : pairVecDivVHCActivateReset resetH heap nodes quotient divisor =
      .ok result) : result.nodes.size = nodes.size := by
  induction resetH using Nat.strong_induction_on generalizing heap nodes result with
  | h resetH ih =>
      rw [pairVecDivVHCActivateReset] at hrun
      by_cases hreset : 0 < resetH
      · simp only [hreset, ↓reduceDIte] at hrun
        cases hactivate : pairVecDivVHCActivate (resetH - 1) nodes quotient
            divisor with
        | error fault => simp [hactivate] at hrun
        | ok activated =>
            ...
            | ok inserted =>
                rcases inserted with ⟨heap', nodes'⟩
                rw [hinsert] at hrun
                have htail := ih (resetH - 1) (by omega) heap' nodes' result hrun
                exact htail.trans ((pairVecDivVHCInsert_nodes_size
                  (resetH - 1) heap heap' activated nodes' hinsert).trans
                    (pairVecDivVHCActivate_nodes_size (resetH - 1) nodes
                      activated quotient divisor hactivate))
      · simp only [hreset, ↓reduceDIte, Except.ok.injEq] at hrun
        subst result
        rfl

theorem pairVecDivVHCEmit_nodes_size
    (this : DenseUPolyZp) (frontier : PairVecDivVHCFrontier)
    (consumed : PairVecDivVHCEqualDegreeResult)
    (quotient divisor quotient' : SparsePolyZp)
    (activated : PairVecDivVHCHeapState) (resetH' : Nat)
    ...
    (hrun : pairVecDivVHCEmit this frontier consumed quotient divisor hdivisor =
      .ok (quotient', activated, resetH')) :
    activated.nodes.size = consumed.nodes.size := by
  unfold pairVecDivVHCEmit at hrun
  by_cases hcoefficient : consumed.coefficient ≠ 0
  · rw [if_pos hcoefficient] at hrun
    by_cases hdegree : divisor[0].1.deg ≤ frontier.degree
    · rw [if_pos hdegree] at hrun
      ...
      · rw [if_pos hvalue] at hrun
        ...
        | ok state =>
            ...
            simp only [Except.ok.injEq, Prod.mk.injEq] at hrun
            rcases hrun with ⟨rfl, rfl, rfl⟩
            exact pairVecDivVHCActivateReset_nodes_size consumed.resetH
              consumed.heap consumed.nodes emitted divisor state hactivate
      · rw [if_neg hvalue] at hrun
        simp only [Except.ok.injEq, Prod.mk.injEq] at hrun
        rcases hrun with ⟨rfl, rfl, rfl⟩
        rfl
...
theorem pairVecDivVHCOuterIteration_nodes_size
    (this : DenseUPolyZp) (dividendIndex : Nat) (heap : Array Nat)
    (nodes : Array PairVecDivVHCNode) (quotient dividend divisor : SparsePolyZp)
    (resetH : Nat) (frontier : PairVecDivVHCFrontier)
    (result : PairVecDivVHCIterationResult) (hdivisor : 0 < divisor.size)
  ...
  rcases pairVecDivVHCOuterIteration_components this dividendIndex heap nodes
    ...
    ⟨consumed, quotient', activated, resetH', reinserted, hconsume, hemit,
      hreinsert, hresult⟩
  rw [hresult]
  exact (pairVecDivVHCReinsertLin_nodes_size activated.heap activated.nodes
    consumed.lin reinserted hreinsert).trans
      ((pairVecDivVHCEmit_nodes_size this frontier consumed quotient divisor
        quotient' activated resetH' hdivisor hemit).trans
...
theorem pairVecDivVHCOuterIteration_preserves_cursorPrefixAbove
    ...
    (hdecrease : frontier.degree < degreeLimit)
    (hcanonical : SparsePolyZp.Canonical p quotient)
    (hbelow : PairVecDivVHCAllActiveNodesBelow activeLimit nodes)
    (hdenotes : ∀ (i : Nat) (node : PairVecDivVHCNode),
      nodes[i]? = some node → node.mono ≠ none →
        PairVecDivVHCNodeDenotes quotient divisor node)
    (hfixed : PairVecDivVHCNodeDivisorIndicesFixed nodes)
  ...
  have hconsumedInvariants :=
    pairVecDivVHCConsumeEqualDegree_preserves_node_invariants this p
      activeLimit frontier.degree heap frontier.coefficient nodes #[] resetH
      quotient divisor consumed owners hcanonical hbelow hdenotes hfixed
      hready hownership hconsume
...
/-- Below the divisor lead degree the generated emit block cannot append a
quotient term, so a complete outer iteration leaves the quotient unchanged. -/
theorem pairVecDivVHCOuterIteration_quotient_eq_of_frontier_lt_lead
    (this : DenseUPolyZp) (dividendIndex : Nat) (heap : Array Nat)
    (nodes : Array PairVecDivVHCNode)
    (quotient dividend divisor : SparsePolyZp) (resetH : Nat)
    (frontier : PairVecDivVHCFrontier)
    ...
    (hrun : pairVecDivVHCOuterIteration this dividendIndex heap nodes quotient
      dividend divisor resetH = .ok result) :
    result.quotient = quotient := by
  rcases pairVecDivVHCOuterIteration_components this dividendIndex heap nodes
      quotient dividend divisor resetH frontier result hdivisor hselect hrun with
    ⟨consumed, quotient', activated, resetH', reinserted, hconsume, hemit,
      hreinsert, hresult⟩
  have hsame := pairVecDivVHCEmit_quotient_eq_or_push this frontier consumed
    quotient divisor quotient' activated resetH' hdivisor hemit
  rcases hsame with hsame | ⟨hlead, _⟩
  · rw [hresult]
    exact hsame
  · omega

/-- Once the well-founded outer-loop bound is at or below the divisor lead,
every later selected frontier is strictly below the lead and the generated
loop returns the current quotient unchanged. -/
theorem pairVecDivVHCOuterLoop_quotient_eq_of_limit_le_lead
    (this : DenseUPolyZp) (degreeLimit dividendIndex : Nat)
    (heap : Array Nat) (nodes : Array PairVecDivVHCNode)
    (quotient dividend divisor output : SparsePolyZp) (resetH : Nat)
    (hdivisor : 0 < divisor.size)
    (hlimit : degreeLimit ≤ divisor[0].1.deg)
    (hrun : pairVecDivVHCOuterLoop this degreeLimit dividendIndex heap nodes
      quotient dividend divisor resetH = .ok output) :
    output = quotient := by
  induction degreeLimit using Nat.strong_induction_on generalizing dividendIndex
      heap nodes quotient resetH with
  | h degreeLimit ih =>
      by_cases hdone : dividend.size ≤ dividendIndex ∧ heap.size = 0
      · rw [pairVecDivVHCOuterLoop] at hrun
        rw [dif_pos hdone] at hrun
      ...
      · cases hselect : pairVecDivVHCSelectFrontier dividendIndex dividend
        ...
        | ok frontier =>
            rcases pairVecDivVHCOuterLoop_step_of_success this degreeLimit
                ...
                frontier output hdone hselect hrun with
              ⟨hdecrease, next, hiteration, hrecursive⟩
            have hnextEq := ih frontier.degree hdecrease next.dividendIndex
              next.heap next.nodes next.quotient next.resetH
              (Nat.le_trans (Nat.le_of_lt hdecrease) hlimit) hrecursive
            exact hnextEq.trans
              (pairVecDivVHCOuterIteration_quotient_eq_of_frontier_lt_lead this
                dividendIndex heap nodes quotient dividend divisor resetH
                frontier next hdivisor (lt_of_lt_of_le hdecrease hlimit)
                hselect hiteration)

/-- The real well-founded generated outer loop computes a quotient whose
product agrees with the dividend at every degree at or above the divisor lead.
Below-lead source iterations are not discarded: they are executed, proved not
to mutate the quotient, and their open sparse gap is discharged from the heap
and dividend cursor invariants. -/
theorem pairVecDivVHCOuterLoop_productAgreesAbove_lead_of_success
    (this : DenseUPolyZp) [Fact (Nat.Prime this._p.toNat)]
    (globalLimit degreeLimit dividendIndex : Nat)
    (heap : Array Nat) (nodes : Array PairVecDivVHCNode)
    (quotient dividend divisor output : SparsePolyZp) (resetH : Nat)
    ...
    (hrun : pairVecDivVHCOuterLoop this degreeLimit dividendIndex heap nodes
      quotient dividend divisor resetH = .ok output) :
    PairVecDivVHCProductAgreesAbove this._p.toNat divisor[0].1.deg output
      dividend divisor := by
  induction degreeLimit using Nat.strong_induction_on generalizing dividendIndex
      heap nodes quotient resetH owners with
  | h degreeLimit ih =>
      ...
      · rw [pairVecDivVHCOuterLoop] at hrun
        rw [dif_pos hdone] at hrun
        have houtput : output = quotient := (Except.ok.inj hrun).symm
        subst output
        intro degree hdegree
        by_cases hold : degreeLimit ≤ degree
        · exact hagrees degree hold
        · have hheapEmpty : heap = #[] :=
            Array.eq_empty_of_size_eq_zero hdone.2
          subst heap
          rw [pairVecDivVHCProduct_coeff_eq_zero_of_done this._p.toNat
              degreeLimit degree resetH quotient divisor nodes owners hdivisor
              hsize hfixed hcovered hownership hready hprefix hprocessed
              (by omega),
            pairVecDivVHCDividend_coeff_eq_zero_of_done this._p.toNat
              degreeLimit degree dividendIndex dividend hdividendCanonical
              hconsumed hdone.1 (by omega)]
      · cases hselect : pairVecDivVHCSelectFrontier dividendIndex dividend
            heap nodes with
        | error fault =>
            rw [pairVecDivVHCOuterLoop] at hrun
        ...
        | ok frontier =>
            rcases pairVecDivVHCOuterLoop_step_of_success this degreeLimit
                ...
                frontier output hdone hselect hrun with
              ⟨hdecrease, next, hiteration, hrecursive⟩
            by_cases hlead : divisor[0].1.deg ≤ frontier.degree
            · have hiterationInv :=
                pairVecDivVHCOuterIteration_preserves_heapChainsOwned this
                  globalLimit dividendIndex heap nodes quotient dividend divisor
                  resetH next owners hdivisor hcfg hcanonical
              ...
              have hnextQuotientReady : ∀ nextFrontier :
                ...
                have hnextDecrease :=
                  pairVecDivVHCOuterLoop_selected_degree_lt_of_success this
                    frontier.degree next.dividendIndex next.heap next.nodes
                    next.quotient dividend divisor output next.resetH
                    nextFrontier hnextSelect hrecursive
                exact
                  pairVecDivVHCOuterIteration_preserves_quotientAbove_of_lt this
                    dividendIndex nextFrontier.degree heap nodes quotient
                    dividend divisor resetH frontier next hdivisor
                    (hquotientReady frontier hselect) hnextDecrease hselect
                    hiteration
              rcases pairVecDivVHCOuterIteration_preserves_heapChainsHomogeneous
                  this this._p.toNat globalLimit dividendIndex heap nodes quotient
                  dividend divisor resetH frontier next owners hdivisor hcanonical
                  hbelow hdenotes hfixed hready hownership hhomogeneous hselect
                  hiteration with ⟨homOwners, hhomOwnership, hnextHomogeneous⟩
              have hnextHomogeneous' := hnextHomogeneous.congr_owners next.heap
                homOwners nextOwners next.nodes hhomOwnership hnextOwnership
              have hnextOrdered :=
                pairVecDivVHCOuterIteration_preserves_heapOrdered this
                  this._p.toNat globalLimit dividendIndex heap nodes quotient
                  dividend divisor resetH frontier next owners hdivisor hcanonical
                  hbelow hdenotes hfixed hready hownership hordered hselect
                  hiteration
              have hnextPrefix :=
                pairVecDivVHCOuterIteration_preserves_cursorPrefixAbove this
                  this._p.toNat globalLimit degreeLimit dividendIndex heap nodes quotient
                  dividend divisor resetH frontier next owners hdivisor hselect
                  hdecrease hcanonical hbelow hdenotes hfixed hready hcovered
                  hownership hhomogeneous hprefix hiteration
              have hnextProcessed :=
                pairVecDivVHCOuterIteration_preserves_quotientLeadAbove this
                  degreeLimit dividendIndex heap nodes quotient dividend divisor
                  resetH frontier next hdivisor hprocessed hdecrease hselect
                  hiteration
              have hnextConsumed :=
                pairVecDivVHCOuterIteration_preserves_consumed_above this
                  degreeLimit dividendIndex heap nodes quotient dividend divisor
                  resetH frontier next hdivisor hconsumed hdecrease hselect
                  hiteration
              have hnextAgrees :=
                pairVecDivVHCOuterIteration_extends_productAgreement this
                  degreeLimit dividendIndex heap nodes quotient dividend divisor
                  resetH frontier next owners hdivisor hsize hcovered hownership
                  hhomogeneous hordered hdenotes hfixed hready hprefix hprocessed
                  hcfg hcanonical hdividendCanonical hdivisorCanonical
                  (hquotientReady frontier hselect) hconsumed hdecrease hlead
                  hagrees hselect hiteration
              have hnextSize : next.nodes.size = divisor.size - 1 := by
                rw [pairVecDivVHCOuterIteration_nodes_size this dividendIndex
                  heap nodes quotient dividend divisor resetH frontier next
                  hdivisor hselect hiteration, hsize]
              exact ih frontier.degree hdecrease next.dividendIndex next.heap
                next.nodes next.quotient next.resetH nextOwners hnextCanonical
                hnextQuotientReady hnextRemaining hnextBelow hnextDenotes
                hnextFixed hnextReady hnextOwnership hnextCovered hnextSize
                hnextHomogeneous' hnextOrdered hnextPrefix hnextProcessed
                hnextConsumed hnextAgrees hrecursive
            · have hnextQuotient :=
                pairVecDivVHCOuterIteration_quotient_eq_of_frontier_lt_lead this
                  dividendIndex heap nodes quotient dividend divisor resetH
                  frontier next hdivisor (by omega) hselect hiteration
              have houtputNext :=
                pairVecDivVHCOuterLoop_quotient_eq_of_limit_le_lead this
                  frontier.degree next.dividendIndex next.heap next.nodes
                  next.quotient dividend divisor output next.resetH hdivisor
                  (by omega) hrecursive
              rw [houtputNext, hnextQuotient]
              intro degree hdegree
              by_cases hold : degreeLimit ≤ degree
              · exact hagrees degree hold
              · rw [pairVecDivVHCProduct_coeff_eq_zero_of_gap
                    this._p.toNat degreeLimit degree dividendIndex resetH
                    dividend quotient divisor heap nodes owners frontier hdivisor
                    hsize hfixed hcovered hownership hhomogeneous hready hordered
                    hdenotes hcanonical hprefix hprocessed (by omega) (by omega)
                    hselect,
