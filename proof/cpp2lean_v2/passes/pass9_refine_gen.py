...
输出：
  ...
  * proof/lean/CLPoly/Refinement/Generated/*.lean（已证明公开契约）
...
VERIFIED_DIR = REFINEMENT_DIR / "Generated"
...
def _emit_verified_contract(info: dict) -> str:
    """Emit a checked public wrapper around an already-proved strict theorem."""
    ...
    return f"""-- Auto-generated by cpp2lean v2 Pass 9 (refine_gen)
-- Do not edit: regenerate this public L1 → L2 refinement contract.

import {contract['proof_import']}
...
def refine_gen_pass(top_funcs: Optional[list[MIRFunc]] = None) -> None:
    ...
    for info in REFINEMENT_MAP.values():
        contract = info.get("verified_contract")
        if contract is None:
            continue
        out_path = VERIFIED_DIR / f"{contract['output_file']}.lean"
        out_path.write_text(_emit_verified_contract(info))
        print(f"  Verified refinement: {out_path}", file=sys.stderr)
