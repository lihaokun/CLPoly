...
> 状态：nl-proof v3（§3.1-3.2 代数路径废弃，§3.3 积分 Parseval 正确路径，零点处理补充）
...
```
2 · log M(f) = (1/2π) ∫ log|f(e^{iθ})|² dθ    [log|x|² = 2·log|x|, Step 1 定义]
             ≤ log((1/2π) ∫ |f(e^{iθ})|² dθ)   [Step 3: Jensen, E[log X] ≤ log E[X]]
...
**零点处理**：若 f 在单位圆上有零点 e^{iθ₀}，则 log|f(e^{iθ₀})| = -∞。
但非零多项式在单位圆上最多有有限个零点（测度 0 集）。
Mathlib 的 `logMahlerMeasure` 定义处理了这个（积分是有限的，由 `mahlerMeasure_pos_of_ne_zero` 保证）。
Jensen 不等式对 a.e. 有限的函数成立：E[log X] = -∞ 时不等式 -∞ ≤ log E[X] 平凡成立。
故零点不影响证明。✓
...
**以下 §3.1-3.2 是错误路径，保留作为反面教训。正确路径从 §3.3 开始。**
`Σ|fₖ|² = |lc|²·∏(1+|αᵢ|²)` 是**错误恒等式**（反例：f=(X-1)(X-2) 给 14≠10）。
...
### 3.3 积分 Parseval（自证，唯一可行路径）

代数路径不可行（`Σ|fₖ|² ≠ |lc|²·∏(1+|αᵢ|²)`，反例：f=(X-1)(X-2) 给 14 ≠ 10）。
回到积分路径。**自证有限 Fourier Parseval**：
