...
2. **分类处理**：
   ...
   - **其他情况**（接口不合理、遗漏场景、设计缺陷、新增功能）— 先完成根因分析（§5.1），再回到设计阶段调整设计文档，最后按实现流程执行
3. **举一反三** — 梳理是否存在其他同类问题，反思问题产生的原因，补充相关测试用例
4. **回归测试** — 将本次发现 bug 的用例补充为回归用例，重新运行所有已有模块的测试，确保无回归

### 5.1 根因分析与修正方案文档（修复前必做）

对于非显而易见的错误，**禁止**跳过分析直接编码修复。必须先产出**修正方案文档**（`docs/<模块名>-fix-<简述>.md`），经用户确认后才可编码。
...
- 对照至少一个成熟参考实现（如 SymPy、FLINT、GCL 教科书）
...
Bug 修复：
  局部错误 → 直接修复 + 补测试 + 回归
  其他情况 → 修正方案文档(§5.1) → [用户确认] → 更新设计 → 按实现流程执行 + 回归
             ↑ 算法类 bug 必须对照参考实现
