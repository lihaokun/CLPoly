...
namespace clpoly{
    ...
    // 字典序的coeff
    // 返回多项式关于first_var的各系数多项式
    // 约定：常数的系数多项式只有一个，为其本身
    template<class Tc,class var_order>
    std::vector<polynomial_<Tc,lex_<var_order>>> coeff(const polynomial_<Tc,lex_<var_order>> &F)
    {
        polynomial_<Tc,lex_<var_order>>  tmp(F.comp_ptr());
        if (is_number(F))
        {
            tmp=F;
            return {tmp};
        }
        // 保证首变量不是variable(), 因为__change_monomial_var_deg中的 v 不能是 variable()
        variable v=get_first_var(F);
        int64_t v_d=get_first_deg(F);
        std::vector<polynomial_<Tc,lex_<var_order>>>  coF(v_d+1,tmp);
        for (auto &i:F)
        {
            // 单项式的首变量是v且次数是v_d 或者 首变量不是v且v_d==0
            if ((get_first_var(i.first)==v && get_first_deg(i.first)==v_d) || (get_first_var(i.first)!=v && v_d==0))
            {
                
                tmp.push_back({__change_monomial_var_deg(i.first,v,0),i.second});
            }
            else{
                // 这里调用移动赋值避免拷贝开销，而且自动清空tmp
                coF[v_d]=std::move(tmp);
                // 项式的首变量是v则更新v_d=次数，否则v_d=0
                v_d=(get_first_var(i.first)==v)?get_first_deg(i.first):0;
                tmp.push_back({__change_monomial_var_deg(i.first,v,0),i.second});
            }
        }
        coF[v_d]=std::move(tmp);
    ...
    // 字典序的coeff
    // 如果是常数多项式或者v小于多项式的首变量，则返回只有一个元素的向量，元素为多项式本身
    // 如果v是首变量，调用上面的coeff字典序版本
    // 否则先转变变量序，再调用上面的coeff字典序版本，最后转变回去
    // 常数的系数多项式只有一个，为其本身
    template<class Tc,class var_order>
    std::vector<polynomial_<Tc,lex_<var_order>>> coeff(const polynomial_<Tc,lex_<var_order>> &F,const variable &v)
    {
        // 如果是常数多项式或者v小于多项式的首变量，则返回只有一个元素的向量，元素为多项式本身
        if (is_number(F) || F.comp().comp(v, get_first_var(F)) )
        {
            // 必须这样做，因为拷贝构造函数不能拷贝项序
            polynomial_<Tc,lex_<var_order>>  tmp(F.comp_ptr());
            tmp=F;
            return {tmp};
        }
        ...
        // 否则先转变变量序，再调用自身，最后转变回去
        lex_<custom_var_order> comp_v({v});
        polynomial_<ZZ,lex_<custom_var_order>> F_(&comp_v);
        // univariate_priority_order comp_v(v);
        // polynomial_<Tc,univariate_priority_order> F_(&comp_v);
        // 这里不能用 F_=F; 因为相同项序类型的 赋值会拷贝项序，不同项序类型的赋值只会拷贝数据
        poly_convert(F,F_);
        auto coeff_l=coeff(F_,v);
        std::vector<polynomial_<Tc,lex_<var_order>>>  coF(coeff_l.size(),polynomial_<Tc,lex_<var_order>>(F.comp_ptr()));
        for (size_t i=0;i<coeff_l.size();++i)
        {
            //  同样不能用 coF[i]=coeff_l[i];
            poly_convert(coeff_l[i],coF[i]);
        }
        return coF;
    }
    ...
    template <class var_order>
    std::vector<std::vector<polynomial_<ZZ,lex_<var_order>>>> __project_mccallum_full(
        const std::vector<polynomial_<ZZ,lex_<var_order>>>& polys, 
    ...
    )
    {
        ...
        for(size_t i=0;i<vars.size();i++)
        {
            ...
            // 计算投影
            current_set=__project_mccallum(allprojs.back(),x);
    ...
    // 旧版本：先算 squarefreebasis, 再算 cont, prim
    // // 字典序的投影算子
    // // 需要传入x, 因为某些多项式可能不含x
    // // 假设x是多项式包含的所有变量中 var_order 最小的变量
    // template <class var_order>
    // std::vector<polynomial_<ZZ,lex_<var_order>>> __project_mccallum_old(
    //     const std::vector<polynomial_<ZZ,lex_<var_order>>>& polys, 
    //     const clpoly::variable& x
    // )
    // {
        
    //     // 先用 squarefreebasis 计算无平方的基，调用 字典序 的实现
    //     auto [sqfree_polys, _] = squarefreebasis(polys);
    //     // 计算cont和prim
    //     // std::vector<polynomial_<ZZ,lex_<var_order>>> projs,prims;
    //     auto [projs,prims]=__conts_prims_polys_var(sqfree_polys,x);
        
    //     // 计算 coeff 和 discriminant
    //     for (const auto& poly : prims) {
    //         // 调用 字典序 的 coeff
    //         auto coF = coeff(poly);
    //         for (auto& c : coF) {
    ...
    // // 假设x是多项式包含的所有变量中 var_order 最小的变量
    // template <class var_order>
    // std::vector<polynomial_<ZZ,lex_<var_order>>> __project_lazard_old(
    //     const std::vector<polynomial_<ZZ,lex_<var_order>>>& polys, 
    //     const clpoly::variable& x
    // )
    // {
    ...
    //         {
    //             auto f_cont=cont(poly);
    //             auto f_prim=poly/f_cont;
    //             // 如果有因子x, 需要分解
    //             if (__has_factor_first_var(f_prim))
    //             {
    //                 // 强制类型转换 x
    //                 polynomial_<ZZ,lex_<var_order>> x_factor(poly.comp_ptr());
    //                 x_factor.push_back({{{x,1}},1});
    ...
    //                 // 原地做除法 f_prim /= x, 把首变量的次数减1, 如果次数为1则删除该变量
    ...
    //             }
    //             if (!is_number(f_prim))
    //                 prims.push_back(std::move(f_prim));
    //             if (!is_number(f_cont))
    //                 projs.push_back(std::move(f_cont));
    //         }
    //         else
    //         {
    //             if (!is_number(poly))
